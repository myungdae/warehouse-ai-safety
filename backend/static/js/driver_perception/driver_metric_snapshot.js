    ...
    function createDriverMetricSnapshot({ frame, metrics, calibration, now = Date.now() }) {
        ...
        const snapshot = {
            ...
            metrics: {
                ...
                longClosureCandidate: metrics.longClosureCandidate === true, longClosureAccepted: metrics.longClosureAccepted === true,
                longClosureEpisodeId: metrics.longClosureEpisodeId || null, longClosureState: metrics.longClosureState || 'IDLE',
                longClosureDurationMs: safeNumber(metrics.longClosureDurationMs), longClosureCount: Number.isInteger(metrics.longClosureCount) ? metrics.longClosureCount : 0,
                longClosureRejectReason: metrics.longClosureRejectReason || null, generation: metrics.generation ?? calibration.runtimeGeneration ?? null,
            ...
            diagnostics: {
                eye: {
                    ...
                    previousFrameTimestamp: Number.isFinite(metrics.previousFrameTimestamp) ? new Date(metrics.previousFrameTimestamp).toISOString() : null,
                    currentFrameTimestamp: frame.timestamp,
                    observedFrameGapMs: safeNumber(metrics.frameDeltaMs),
                    configuredMaxFrameGapMs: safeNumber(namespace.Config?.metrics?.maxFrameGapMs),
                    frameTimestampSource: frame.timestampSource || 'UNKNOWN',
                    processingLatencyMs: safeNumber(frame.callbackLatencyMs ?? frame.processingTimeMs),
                    droppedFrameCount: Number.isInteger(frame.droppedResultEstimate) ? frame.droppedResultEstimate : 0,
                    blinkCandidateStart: metrics.leftCloseStartedAt == null && metrics.rightCloseStartedAt == null ? null
                        : new Date(Math.min(...[metrics.leftCloseStartedAt, metrics.rightCloseStartedAt].filter(Number.isFinite))).toISOString(),
                    blinkCandidateEnd: metrics.recoveryStartedAt == null ? null : new Date(metrics.recoveryStartedAt).toISOString(),
                    frameGapComparison: Number.isFinite(metrics.frameDeltaMs)
                        ? `${metrics.frameDeltaMs} > ${namespace.Config?.metrics?.maxFrameGapMs} = ${metrics.frameDeltaMs > namespace.Config?.metrics?.maxFrameGapMs}` : 'NO_PREVIOUS_FRAME',
            ...
            runtime: { processingTimeMs: safeNumber(frame.processingTimeMs), frameTimestamp: frame.timestamp,
                captureTimestampMs: safeNumber(frame.captureTimestampMs), submissionTimestampMs: safeNumber(frame.submissionTimestampMs),
                resultCallbackTimestampMs: safeNumber(frame.resultCallbackTimestampMs), timestampSource: frame.timestampSource || 'UNKNOWN',
                callbackLatencyMs: safeNumber(frame.callbackLatencyMs), frameSequence: Number.isInteger(frame.frameSequence) ? frame.frameSequence : null,
                duplicateResult: frame.duplicateResult === true, duplicateResultCount: Number.isInteger(frame.duplicateResultCount) ? frame.duplicateResultCount : 0,
                droppedResultEstimate: Number.isInteger(frame.droppedResultEstimate) ? frame.droppedResultEstimate : 0 },
